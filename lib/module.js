	...
	constructor(client, options) {
		...
		listen$.subscribe(async message => {
			...
			try {
				...
				if (!user) {
					logger.info(`[${nick}] User not found.`);
				...
				if (!track || !track.name || !track.artist) {
					logger.info(`[${nick}] Track lookup failed.`);
				...
				let text = [track_string];

				if (track['@attr']?.['nowplaying'] == 'true') {
					text.unshift(`${user} is listening to:`);
				} else {
					text.unshift(`${user} last played:`);
				}

				logger.info(`[${nick}] ${track_string}`);
				...
				logger.error(`[${nick}] ${error.message}`);
